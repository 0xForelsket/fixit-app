...
function migrateTestContent(content: string): { content: string; changes: string[] } {
  ...
  if (result.includes('import { render, screen }')) {
    ...
    changes.push("Removed 'screen' from imports (use render destructuring instead)");
  }

  // Also handle screen-only imports
  if (result.includes('from "@testing-library/react"') && result.includes('screen')) {
    result = result.replace(
  ...
  if (result.includes('screen.getBy') || result.includes('screen.queryBy') || result.includes('screen.findBy')) {
    changes.push("WARNING: Manual update needed - replace screen.X() with destructured queries from render()");
  ...
  if (result.includes('userEvent.setup()')) {
    changes.push("WARNING: Manual update needed - replace userEvent.click() with element.click()");
  ...
  const attrRegex = /expect\(([^)]+)\)\.toHaveAttribute\(["']([^"']+)["'],\s*["']([^"']+)["']\)/g;
  ...
  if (disabledRegex.test(result)) {
    ...
    changes.push("Replaced .toBeDisabled() with .hasAttribute('disabled').toBe(true)");
  }
  ...
  if (classRegex.test(result)) {
    ...
    changes.push("Replaced .toHaveClass() with .classList.contains().toBe(true)");
  }

  return { content: result, changes };
}

async function processFile(filePath: string, dryRun: boolean): Promise<MigrationResult | null> {
...
async function main() {
  ...
  if (totalChanges > 0) {
    ...
    console.log('      const { getByRole, getByText } = render(<Component />);');
    console.log('   2. Replace screen.getByX() with getByX()');
    console.log('   3. Replace userEvent.click(el) with el.click()');
    console.log('   4. Run tests to verify: bun test ./src/tests/unit/components/');
  }
}
