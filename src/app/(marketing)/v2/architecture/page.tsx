          ...
          <div className="relative z-10 space-y-6">
            ...
            <ul className="space-y-4 pt-4 text-xs font-mono font-bold tracking-widest uppercase">
              <li className="flex items-center gap-3">
                  <motion.div initial={{ width: 0 }} whileInView={{ width: 6 }} className="h-1.5 bg-primary-foreground rounded-full" />
                  Zero Vendor Lock-in
              </li>
              <li className="flex items-center gap-3">
                  <motion.div initial={{ width: 0 }} whileInView={{ width: 6 }} className="h-1.5 bg-primary-foreground rounded-full" />
...
function TopologyBlock({ icon: Icon, title, code, isTextOnly }: { icon: any; title: string, code: string, isTextOnly?: boolean }) {
  return (
    <motion.div 
    ...
    >
      <div className="flex items-center gap-2 text-primary mb-3">
        ...
        <span className="font-bold text-sm tracking-tight text-foreground/80">{title}</span>
      </div>
      <div className={`opacity-60 group-hover:opacity-100 transition-opacity ${isTextOnly ? 'font-sans italic text-sm' : ''}`}>
        {code.split('\n').map((line, i) => (
          <div key={i}>{line}</div>
        ))}
      </div>
...
function StackCard({
  ...
  return (
    ...
    >
      ...
      <div>
        <h3 className="font-mono text-sm font-black uppercase tracking-widest text-primary mb-1">{title}</h3>
        <p className="text-xs font-mono text-muted-foreground uppercase tracking-tighter opacity-70 mb-3">{spec}</p>
