import { db } from "@/db";
import { equipment, notifications, users, workOrders } from "@/db/schema";
...
export default async function AdminDashboardPage() {
  ...
  const equipmentCount = await db.select({ count: count() }).from(equipment).then(res => res[0].count);
      ...
      <div className="border-t border-zinc-100 pt-8">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {quickLinks.map((link) => (
                ...
                >
                    <div className={cn("h-14 w-14 rounded-xl flex items-center justify-center transition-transform group-hover:scale-110", link.bg)}>
