...
import { validEquipmentCSV, invalidCSV } from "./fixtures/import-data";
...
test.describe("Admin - Import Wizard", () => {
  ...
  test("Admin can access import page and select resource", async ({ page }) => {
    ...
    await expect(page.getByText("What would you like to import?")).toBeVisible();

    // Select Equipment
    await page.getByRole("button", { name: /Equipment/i }).click();
    await expect(page.getByRole("heading", { name: "Import Equipment" })).toBeVisible();
    await expect(page.getByText("Drop your CSV file here")).toBeVisible();
  });

  test("Shows error for invalid CSV format during preview", async ({ page }) => {
    await page.goto("/admin/import");
    await page.getByRole("button", { name: /Equipment/i }).click();
  ...
  test("Successfully previews and validates equipment import", async ({ page }) => {
    await page.goto("/admin/import");
    await page.getByRole("button", { name: /Equipment/i }).click();

    const uniqueId = Date.now().toString().slice(-6);
    const csvWithUniqueCodes = validEquipmentCSV.replace(/EQ-E2E/g, `EQ-V-${uniqueId}`);
  ...
  test("Successfully imports equipment records", async ({ page }) => {
    ...
    const uniqueId = Date.now().toString().slice(-6);
    const csvWithUniqueCodes = validEquipmentCSV.replace(/EQ-E2E/g, `EQ-I-${uniqueId}`);
...
USER-E2E-${uniqueId},E2E User ${uniqueId},e2e-${uniqueId}@example.com,1234,tech,45.00`;
  ...
  test("Can download sample CSV template", async ({ page }) => {
    ...
    expect(download.suggestedFilename()).toContain("locations-import-template.csv");
  });
  ...
  test("Can navigate back and select different resource", async ({ page }) => {
    ...
    // Select Users
    await page.getByRole("button", { name: /Users/i }).click();
    await expect(page.getByRole("heading", { name: "Import Users" })).toBeVisible();
  });
});
...
test.describe("Permissions - Import Wizard", () => {
  test("Tech cannot access import page", async ({ page, loginAsTech }) => {
    ...
    await expect(page.getByText("What would you like to import?")).not.toBeVisible();
  });
});
