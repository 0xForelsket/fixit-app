...
export function StatsCard({
  ...
  const DefaultContent = (
    <>
      ...
      <div className="space-y-1">
        ...
        <h3 className={cn("text-2xl sm:text-3xl font-black tracking-tight", color)}>
          {typeof value === "number" ? String(value).padStart(2, "0") : value}
        </h3>
        {description && (
             <p className="text-xs text-zinc-500 font-medium">{description}</p>
  ...
  // Container Classes
  const baseClasses = "relative overflow-hidden rounded-2xl border bg-white transition-all duration-300";
  
  const variantClasses = {
      default: cn(
          "flex flex-col justify-between p-3 card-industrial shadow-sm",
          border,
          pulse && typeof value === "number" && value > 0 && "animate-glow-pulse border-danger-300",
