  ...
  return (
    <TreeExplorer
      ...
      renderIcon={(_item, hasChildren) => (
        hasChildren ? <Layers className="h-5 w-5" /> : <MonitorCog className="h-5 w-5" />
      )}
      renderTitle={(item) => (
      ...
      renderActions={(item) => (
        <>
          <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg" asChild title="Add Sub-asset">
            <Link href={`/assets/equipment/new?parentId=${item.id}&locationId=${item.locationId}`}>
      ...
      getSearchTerms={(item) => [item.name, item.code, item.location?.name || ""]}
