...
test.describe("Technician - Work Orders", () => {
  ...
  test("Tech can view work order list and search", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /Work Order Queue/i })).toBeVisible();
  ...
  test("Tech can interact with checklist and persistence", async ({ page }) => {
    ...
    const workOrderRow = page.locator('tr:has-text("Maintenance"), tr:has-text("Calibration")').first();
    const isVisible = await workOrderRow.isVisible({ timeout: 5000 }).catch(() => false);
    ...
    const hasProcedure = await procedureHeader.isVisible({ timeout: 3000 }).catch(() => false);
    
    if (!hasProcedure) {
      console.log("Work order has no procedure attached, skipping checklist test");
      return;
    }

    // Find first checklist item toggle button within the Procedure section
    const procedureSection = page.locator('div:has-text("Maintenance Procedure")');
  ...
  test("Tech can log time using the timer", async ({ page }) => {
    ...
    // Stop timer
    const responsePromise = page.waitForResponse(res => res.url().includes('/api/labor') && res.status() === 201);
  ...
  test("Tech can add manual labor entry", async ({ page }) => {
    ...
    const manualBtn = page.locator('button').filter({ has: page.locator('svg.lucide-plus') }).first();
    ...
    await expect(page.getByText(uniqueNote).locator('..').locator('..').getByText("45m")).toBeVisible();
  });

  test("Tech can transition work order status and add comments", async ({ page }) => {
    ...
    // Resolve Work Order
    const resolveBtn = page.getByRole("button", { name: /Resolve Work Order/i });
    if (await resolveBtn.isVisible()) {
      await resolveBtn.click();
      
      const notesArea = page.getByPlaceholder("What was done to fix the issue?");
