...
interface StatusConfig {
  variant: "default" | "secondary" | "destructive" | "outline" | "success" | "warning" | "danger" | "critical";
  label?: string;
...
export function StatusBadge({
  ...
  const dotColorClass = config.variant === 'danger' || config.variant === 'critical' 
    ? "bg-white" 
    : getDotColor(config.variant === 'outline' ? getMappedVariantForOutline(normalizedStatus) : config.variant);
  ...
  return (
    <Badge
      ...
      className={cn(
        ...
        shouldAnimate && config.variant === 'critical' && "animate-pulse shadow-glow-danger", // Whole badge pulse
        className
      )}
    >
      {/* 1. Icon Render */}
