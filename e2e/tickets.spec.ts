...
test.describe("Ticket Management", () => {
  test.describe("Create Ticket (Operator)", () => {
    test("should create a new ticket", async ({ page, loginAsOperator }) => {
      ...
      await page.click('button[role="combobox"]:has-text("Type"), select[name="type"]');
      ...
      await expect(page.locator("text=success, text=created")).toBeVisible({ timeout: 10000 });
    });
  });

  test.describe("Manage Tickets (Tech)", () => {
    test("should view ticket list on dashboard", async ({ page, loginAsTech }) => {
      await loginAsTech();
    ...
    test("should resolve a ticket", async ({ page, loginAsTech }) => {
      ...
      // Find an open ticket
      const openTicket = page.locator('a[href*="/tickets/"]:has-text("Open")').first();
      if (await openTicket.isVisible()) {
        ...
        if (await resolveBtn.isVisible()) {
          ...
          await page.fill('textarea[name="resolutionNotes"]', "Fixed via E2E test");
          
          // Submit
          await page.click('button[type="submit"]:has-text("Resolve")');
          
          // Should show resolved status
          await expect(page.locator("text=Resolved")).toBeVisible({ timeout: 5000 });
        }
      }
    });
  });
